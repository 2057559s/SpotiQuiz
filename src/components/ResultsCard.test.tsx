...
describe("ResultsCard", () => {
  ...
  it("labels the result and routes each action to its callback", () => {
    ...
    expect(screen.getByText("80% accuracy. Your answers are already saved.")).toBeInTheDocument();
  ...
  it("copies a shareable score when native sharing is unavailable", async () => {
    ...
    await vi.waitFor(() => expect(navigator.clipboard.writeText).toHaveBeenCalledWith(expect.stringContaining("2/4 (50%)")));
  });
});
